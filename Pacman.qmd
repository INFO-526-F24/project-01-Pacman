...
ggplot(faceted_data, aes(x = season, y = category, fill = total_sales)) +
  geom_tile() +
  facet_wrap(~region) +
  ...
  scale_fill_gradient(
    low = "white", 
    high = "blue", 
    name = "Total Sales",   # Title for the legend
    breaks = scales::pretty_breaks(n = 5),  # Breaks at nice intervals
    labels = scales::label_comma()  # Format labels as commas for readability
  ) +
  theme_minimal() +
